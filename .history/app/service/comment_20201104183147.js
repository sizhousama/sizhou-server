 ...
 * @LastEditors: sizhou
 * @LastEditTime: 2020-11-04 18:31:46
 */
'use strict';
...
class Comment extends Service {
  async comments({ page, size, articleId }) {
    ...
    await this.ctx.model.Comment.findAndCountAll({
      ...
      rows.forEach(one => {
        console.log(one);
        one.dataValues.replys = [];
        list.forEach(item => {
          if (item.dataValues.parent_id === one.dataValues.id) {
            one.dataValues.replys.push(item);
            this.ctx.model.User.findOne({
              where: { id: item.reply_id },
              attributes: [ 'id', 'username', 'profession', 'avatar', 'total_like' ],
            }).then(res => {
              item.dataValues.reply_user = res;
            });
          }
        });
        one.dataValues.reply_count = one.dataValues.replys.length;
      });
      comments = rows;
    });
